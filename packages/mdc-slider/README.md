...
For discrete slider, it requires to have a step value other than `0`. So if a step is not provided,
 or `0` is provided, it will reset the step value to be `1`.
...
`mdc-slider--display-markers` to `mdc-slider` and
`<div class="mdc-slider__track-marker-container"></div>` to track container.
...
> we place the secondary last marker proportionally at where thumb could reach and
