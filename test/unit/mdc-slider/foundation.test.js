...
test('#setupTrackMarker append right number of markers to discrete slider with markers', () => {
  const {foundation, mockAdapter} = setupTest();
  const raf = createMockRaf();
...
test('#setMax re-place track markers if slider is discrete and displays markers', () => {
  const {foundation, mockAdapter} = setupTest();
  const raf = createMockRaf();
  const {isA} = td.matchers;
